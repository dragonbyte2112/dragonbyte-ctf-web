...
export default async function handler(req, res) {
  ...
  if (req.method === "DELETE") {
    ...
    try {
      ...
      // Clean up any files attached to this room's questions before wiping Firestore data
      const qSnap = await ref.collection("questions").get();
      const filePaths = qSnap.docs.map(d => d.data().file_path).filter(Boolean);
      if (filePaths.length) {
        const bucket = getStorage().bucket();
        await Promise.all(filePaths.map(p => bucket.file(p).delete({ ignoreNotFound: true })));
      }
