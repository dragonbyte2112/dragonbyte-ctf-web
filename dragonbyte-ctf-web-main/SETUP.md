...
| `api/admin_challenge_file.js` | POST/DELETE /api/admin_challenge_file — admin upload/remove a challenge's attached file |
| `api/challenge_file.js` | GET /api/challenge_file?id=... — download a challenge's attached file |
| `api/admin_room_question_file.js` | POST/DELETE /api/admin_room_question_file — admin upload/remove a room question's attached file |
| `api/room_question_file.js` | GET /api/room_question_file?roomId=...&questionId=... — download a room question's attached file |
...
### 1f. Enable Firebase Storage (for challenge file attachments)
1. In the sidebar: **Storage** → **Get started**
2. Choose **Production mode**, same region as Firestore → **Done**
3. Note the bucket name shown at the top of the Storage page — this is your
   `FIREBASE_STORAGE_BUCKET` value. Newer projects default to
   `your-project-id.firebasestorage.app` (the code's default); older projects
   use `your-project-id.appspot.com` — if yours does, set
   `FIREBASE_STORAGE_BUCKET` explicitly in Step 2c below.
4. This lets admins (see `ADMIN_EMAILS` below) attach a downloadable file
   (pcap, zip, binary, etc., up to 4MB) to any challenge, which players can
   download from the challenge card.

### 1g. Get Service Account credentials
...
| `FIREBASE_STORAGE_BUCKET` | *(optional)* Only set if your Storage bucket isn't `<project_id>.firebasestorage.app` (e.g. older projects using `<project_id>.appspot.com`) |
| `ADMIN_EMAILS` | Comma-separated emails allowed to manage rooms and challenge file attachments |
