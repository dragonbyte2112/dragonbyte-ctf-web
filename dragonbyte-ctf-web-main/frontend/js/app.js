...
function renderChallengeArea() {
  ...
  const fileRowHtml = (ch.file_name || roomState.isAdmin) ? `
    <div class="challenge-file-row">
      ${ch.file_name
        ? `<a class="btn btn-primary" href="/api/challenge_file?id=${encodeURIComponent(ch.id)}" target="_blank" rel="noopener">
             📎 Download ${esc(ch.file_name)}${ch.file_size ? ` (${formatFileSize(ch.file_size)})` : ""}
           </a>
           ${roomState.isAdmin ? `<button class="btn" style="background:#ff3b5c;color:#fff" onclick="deleteChallengeFile('${ch.id}')">Remove File</button>` : ""}`
        : (roomState.isAdmin ? `<span class="ch-meta-note">No file attached.</span>` : "")
      }
      ${roomState.isAdmin ? `
        <div class="admin-file-upload">
          <input type="file" id="challenge-file-input">
          <button class="btn" id="challenge-file-upload-btn" onclick="uploadChallengeFile('${ch.id}')">⬆️ Upload File</button>
        </div>` : ""
      }
    </div>` : "";
...
// ── Challenge file attachment (admin upload / everyone download) ─────
...
async function uploadChallengeFile(challengeId) {
  ...
  const btn = document.getElementById("challenge-file-upload-btn");
  btn.disabled    = true;
  ...
  try {
    ...
    if (state.challenge && state.challenge.id === challengeId) {
      state.challenge.file_name = file.name;
      state.challenge.file_size = file.size;
      renderChallengeArea();
    }
  } catch (err) {
...
async function deleteChallengeFile(challengeId) {
  ...
  try {
    ...
    if (state.challenge && state.challenge.id === challengeId) {
      state.challenge.file_name = null;
      state.challenge.file_size = null;
      renderChallengeArea();
    }
  } catch (err) {
    toast(err.message, "error");
